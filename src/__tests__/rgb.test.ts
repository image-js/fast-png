...
describe('rgb', () => {
  ...
  it('1 bit', () => {
    ...
    assert.strictEqual(Buffer.from(view).toString("hex"), "000001000001000001000002000002000001000002000002")
  });
  ...
  it('2 bit', () => {
    ...
    assert.strictEqual(Buffer.from(view).toString("hex"), "000001000002000003000004")
  });
  ...
  it('4 bit', () => {
    ...
    assert.strictEqual(Buffer.from(view).toString("hex"), "000001000002000003000004000005000006000007000008")
  });
  ...
  it("convert palette.png tp simple file", () => {
    ...
    const data = rgb(img.data as Uint8Array, img.depth as IndexedColorBitDepth, img.palette)
    const newImg = encode({
      width: img.width,
      height: img.height,
      channels: 3,
